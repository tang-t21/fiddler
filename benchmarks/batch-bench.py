...
if __name__ == "__main__":
    ...
    # for input_length in [16, 32, 64, 128]:
    #     for output_length in [16, 32, 64, 128, 256, 512]:
    # for batch_size in [1, 2, 4, 8, 16]:
    for input_length in [32]:
        for output_length in [32]:
            for batch_size in [1, 2, 4, 8, 16]:
                input_texts = []
                idx_text = 0
                while True:
                ...
                prefill_time_sum, decode_time_sum, hit_rate_sum = 0, 0, 0
                print(
                    f"input_token: {input_length}, output_token: {output_length}, batch_size: {batch_size}"
                )
                ...
                # write to file
                with open(f"./batched-bench-result.txt", "a") as f:
                    f.write(
                        f"{input_length},{output_length},{batch_size},{prefill_time_sum/n_sample:.3f},{decode_time_sum/n_sample:.3f},{hit_rate_sum/n_sample:.4f}\n"
                    )
