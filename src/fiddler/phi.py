...
class FiddlerPhi:
    ...
    def generate(
        ...
        self.outliners = []
        self.expert_pattern = []
        self.expert_counts = [[0 for _ in range(self.n_expert)] for _ in range(self.n_layer)]
        self.clear_cache()
        ...
        for i_layer, layer in enumerate(self.model.layers):
            ...
            if self.cpu_offload == 0:
                ...
                for i_expert in gpu_experts:
                    ...
                    if self.is_expert_in_gpu(i_layer, i_expert):
                        current_state = experts[i_expert](current_state) *routing_weights[top_2, idx_, None]
                    else:
                        # self.expert_placeholder.load_state_dict(
                        #     experts[i_expert].state_dict()
                        # )
                        for name in ["w1", "w2", "w3"]:
                            ...
                            dst.copy_(src)
                        current_state = self.expert_placeholder(current_state) * routing_weights[top_2, idx_, None]
                    inps_after_experts.index_add_(
                        0,
                ...
                for i_expert in cpu_experts:
                    ...
                    current_state = self.run_expert_at_cpu(
                        ...
                        routing_weights[top_2, idx_, None].to("cpu"),
                    )
                    inps_after_experts.index_add_(
                        0,
                        top_2s[i_expert].to(self.dev, non_blocking=True),
    ...
    def get_KV_cache_size(self):
        total_size = 0
        for i in range(len(self.past_key_value)):
            key, value = self.past_key_value[i]
            total_size += key.element_size() * key.nelement()
